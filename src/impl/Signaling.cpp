...
class Signaling : public SignalingChannelInterface {
...
public:
	...
	void processListChannels(int peerid, Json::Value message){
		...
		std::vector<std::string> channels = peer->getChannelNames();
	...
	void processICECandidate(int peerid, Json::Value message){
		...
		peer->onRemoteICECandidate(peerid, sdp_mid, sdp_mlineindex, sdp);
	}
	...
	void processConnect(int peerid, Json::Value message){
		...
		peer->onConnectionRequest(peerid, channels, turn_url, turn_username, turn_password);
	}
	...
	void processAnswer(int peerid, Json::Value message) {
		...
		peer->onRemoteSDP(peerid, type, sdp);
	}
	...
	void processChannelsStreams(int peerId, Json::Value message) {
		...
		for(int i=0; i<_v_mappings.size(); i++) {
			...
			peer->addChannelStreamMapping(peerId, channel, stream);
		}
	}
...
// Factory
SignalingInterface * SignalingInterface::create(){	return new Signaling(); }
